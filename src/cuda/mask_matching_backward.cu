...
torch::Tensor mask_matching_backward(
    ...
    // Make sure all tensors are contiguous and have the expected dtypes.
    auto logits_contig = logits.contiguous();
    auto targets_contig = targets.to(torch::kLong).contiguous();
    auto matches_contig = matches.contiguous();
    auto grad_mask_mean_contig = grad_layer_mask_mean.contiguous();
    auto grad_dice_mean_contig = grad_layer_dice_mean.contiguous();

    const int64_t L = logits_contig.size(0);
    const int64_t B = logits_contig.size(1);
    const int64_t C = logits_contig.size(2);
    const int64_t H = logits_contig.size(3);
    const int64_t W = logits_contig.size(4);

    const int64_t H_t = targets_contig.size(1);
    const int64_t W_t = targets_contig.size(2);
    ...
    const int64_t GT_out = matches_contig.size(2);
    TORCH_CHECK(GT_out > 0 || targets_contig.numel() == 0, "matches must have a non-zero last dimension");
    ...
    int64_t GT_total = 0;
    if (targets_contig.numel() > 0) {
        GT_total = targets_contig.max().item<int64_t>() + 1;
    }
    if (GT_total == 0) {
        return grad_logits;
    }
    ...
    auto counts = torch::zeros({B, GT_total, H, W}, logits_contig.options().dtype(torch::kUInt8));
        ...
        auto static_launcher = [&](auto H_val, auto W_val, auto H_t_val, auto W_t_val) {
            count_labels_per_block_kernel<
                ...
                decltype(W_t_val)::value><<<grid, block>>>(
                    targets_contig.data_ptr<int64_t>(),
    ...
    mask_matching_backward_kernel<<<grad_grid, GRAD_THREADS>>>(
        ...
        matches_contig.data_ptr<int64_t>(),
        counts.data_ptr<uint8_t>(),
        grad_mask_mean_contig.data_ptr<float>(),
        grad_dice_mean_contig.data_ptr<float>(),
